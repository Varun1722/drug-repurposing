...
The user's message text is treated as the disease.
...
async def run_pipeline(ctx: Context, disease: str) -> Dict[str, Any]:
    """
    Run full TS API pipeline and return generated paper and intermediate artifacts.
    """
    return await run_pipeline_with_updates(ctx, disease, None)
...
async def run_pipeline_with_updates(
    ...
    ctx.logger.info(f"Pipeline start for disease='{disease}'")
    ...
    report_data = await asyncio.to_thread(
        post_json,
        "/api/report",
        {"disease": disease, "allResults": all_results, "targets": targets},
        240,
    )
...
async def handle_message(ctx: Context, sender: str, msg: ChatMessage):
    ...
    Message text is interpreted as disease.
    """
    try:
        has_payload = any(
        ...
        disease = extract_text(msg).strip()
        ...
        if normalized in {"paper", "/paper", "resend paper", "show paper"}:
            ...
            if not cached_paper:
                await send_text_with_retry(
                    ctx,
                    sender,
                    "No cached paper found yet for this chat. Run a disease query first.",
                )
                return
        ...
        if not disease:
            await send_text_with_retry(
                ctx,
                sender,
                "Please send a disease as plain text (example: 'triple negative breast cancer', 'Alzheimer's disease').",
            )
            return

        total = ctx.storage.get("total_requests") or 0
        ctx.storage.set("total_requests", total + 1)
        ctx.logger.info(f"New request from {sender[:12]}... disease='{disease}'")

        await send_text_with_retry(
            ctx,
            sender,
            (
                f"Starting pipeline for disease: {disease}\n"
        ...
        result = await run_pipeline_with_updates(ctx, disease, send_progress_update)
