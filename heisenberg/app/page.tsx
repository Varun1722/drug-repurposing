...
export default function Home() {
    ...
    async (userQuery: string) => {
      ...
      try {
        ...
        const reviewRes = await fetch("/api/review", {
          ...
          body: JSON.stringify({ disease: userQuery }),
        })
  ...
  const handleStartSimulation = useCallback(async () => {
    ...
    addMessage(
      "assistant",
      `Starting GNINA molecular docking for ${currentSelectedIds.size} drug${currentSelectedIds.size === 1 ? "" : "s"} across ${dockTargets.length} protein target${dockTargets.length === 1 ? "" : "s"}...`
    )
    ...
    try {
      ...
      if (expandRes.ok) {
        ...
        if (newTargets.length > 0) {
          ...
          addMessage(
            "assistant",
            `Found ${newDrugEntries.length} similar compounds! They've been added to the table and auto-selected. Review them and click "Run GNINA" when ready.`,
            [
              {
                id: "dock-selected",
          ...
          // Set phase to review-ready so checkboxes + Run GNINA button appear
          setPhase("review-ready")
          setStatusMessage("")
        } else {
          addMessage(
  ...
  const handleActionClick = useCallback(
    (action: ChatAction) => {
      ...
      if (val.startsWith("query:")) {
        ...
        addMessage(
          "assistant",
          "Use the checkboxes in the drug table to select which candidates to dock, then click the \"Run GNINA\" button when ready."
        )
